...
# ── Gap-OVL: schedule-overlap walker categories ─────────────────────────────
# Used by _schedule_overlap to classify ops for the exposed-control/sync
# overlap deficit analysis.  See .omc/plans/gap_ovl_wiring_handoff.md §3.1.
...
class ComponentAttribution:
    ...
    Gap-OVL (exposed control/sync overlap deficit): the DES schedule assumes
    control overlaps compute (model_exposed_control_frac), but hardware may
    serialize it (measured_scalar_frac).  The difference is the dominant
    author-headroom mechanism for chunk_kda-class kernels.

    When both rank the same engine first and the bound binds a *different*
    component, ``mis_binding`` is set.
    """
    ...
    measured_scalar_frac: Optional[float] = None         # blended AIV+AIC over wall-clock (dominance)
    measured_aiv_scalar_frac: Optional[float] = None     # same-core scalar share (Gap-OVL pts)
    dominant_measured_engine: Optional[str] = None

    # Gap-OVL fields (populated by _schedule_overlap; None when unavailable)
    critical_path_cycles: Optional[int] = None
    model_exposed_control_frac: Optional[float] = None   # of critical path
    control_busy_frac: Optional[float] = None             # structural fraction
    n_sync_ops: Optional[int] = None
    gap_ovl_pts: Optional[float] = None                   # measured - model (points)
    gap_ovl_us: Optional[float] = None                    # µs, capped at headroom
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "measured_aiv_scalar_frac": self.measured_aiv_scalar_frac,
            "dominant_measured_engine": self.dominant_measured_engine,
            "critical_path_cycles": self.critical_path_cycles,
            "model_exposed_control_frac": self.model_exposed_control_frac,
            "control_busy_frac": self.control_busy_frac,
            "n_sync_ops": self.n_sync_ops,
            "gap_ovl_pts": self.gap_ovl_pts,
            "gap_ovl_us": self.gap_ovl_us,
...
def _ovl_category(op) -> str:
    """Classify an OpRecord for the schedule-overlap walk.

    Returns one of 'control_sync', 'compute', 'memory', or 'other'.
    Uses pipe + op_name (OpRecord does not carry is_sync/is_barrier).
    """
...
def _schedule_overlap(operations) -> dict:
    """Sweep the DES schedule to quantify exposed vs overlapped control.

    Ports the algorithm from scripts/overlap_walker_prototype.py sweep().
    Builds +1/-1 events per category at op start/end boundaries, then walks
    the timeline to classify each interval.

    Args:
        operations: list of OpRecord with start_cycle/end_cycle populated.

    Returns:
        dict with keys: critical_path, exposed_control_cycles,
        control_overlapped_cycles, n_sync_ops, control_busy_frac.
        All zero when the schedule is degenerate (max end_cycle == 0).
    """
    # Count sync ops
    ...
    if not operations:
        return {"critical_path": 0, "exposed_control_cycles": 0,
                "control_overlapped_cycles": 0, "n_sync_ops": n_sync,
                "control_busy_frac": 0.0}
    ...
    events: dict[int, collections.Counter] = collections.defaultdict(
        lambda: collections.Counter()
    )
    for op in operations:
        s, e = op.start_cycle, op.end_cycle
        if e <= s:
            continue
    ...
    state_cycles: collections.Counter = collections.Counter()
    prev = None
    for t in times:
    ...
    for cats, cyc in state_cycles.items():
        ...
        if has_c:
            control_busy += cyc
            if not has_comp and not has_mem:
                exposed_control += cyc
            else:
                control_overlapped += cyc

    control_busy_frac = control_busy / total if total > 0 else 0.0

    return {
        "critical_path": critical_path,
        "exposed_control_cycles": exposed_control,
        "control_overlapped_cycles": control_overlapped,
        "n_sync_ops": n_sync,
        "control_busy_frac": control_busy_frac,
    }
...
def attribute_by_component(
    ...
    Also runs the schedule-overlap walk (_schedule_overlap) to compute the
    Gap-OVL diagnostic: exposed control/sync overlap deficit.
    ...
    Args:
        ...
        t_bound_dsl_us: DSL bound (for capping gap_ovl_us at author headroom).
        t_measured_us: Measured wall-clock time (for gap_ovl_us estimate).

    Returns:
        ComponentAttribution with structural + (optional) measured views +
        Gap-OVL overlap diagnostic.
    """
    cycles: dict[str, int] = {}
    ...
    # ── Schedule-overlap walk (Gap-OVL structural primitives) ──
    ovl = _schedule_overlap(extract.operations)
    ca.critical_path_cycles = ovl["critical_path"]
    ca.n_sync_ops = ovl["n_sync_ops"]
    ca.control_busy_frac = ovl["control_busy_frac"]
    if ovl["critical_path"] > 0:
        ca.model_exposed_control_frac = (
            ovl["exposed_control_cycles"] / ovl["critical_path"]
        )
    ...
    if measured is not None and getattr(measured, "populated", False):
        ...
        # Same-core scalar share for the Gap-OVL subtraction; fall back to the
        # blended frac if the parser predates the aiv_scalar_frac field.
        ca.measured_aiv_scalar_frac = getattr(
            measured, "aiv_scalar_frac", None)
        if ca.measured_aiv_scalar_frac is None:
            ca.measured_aiv_scalar_frac = measured.scalar_frac
    ...
    # ── Gap-OVL metric (denominator-honest) ──
    # Subtract the *same-core* measured scalar share (scalar(AIV)/aiv_time) from
    # the model's critical-path exposed-control fraction — both normalize to one
    # core's timeline.  The blended measured_scalar_frac (over wall-clock) is a
    # different denominator and would over-state the gap.
    if (ca.measured_aiv_scalar_frac is not None
            and ca.model_exposed_control_frac is not None):
        ca.gap_ovl_pts = ca.measured_aiv_scalar_frac - ca.model_exposed_control_frac
        # Secondary µs estimate — capped at author headroom
        if (t_measured_us is not None and t_bound_dsl_us is not None):
            author_headroom_us = t_measured_us - t_bound_dsl_us
            raw = max(0.0, ca.gap_ovl_pts) * t_measured_us
    ...
    if binding_component and consensus_engine and consensus_engine != binding_component.lower():
        ...
        if consensus_engine == "scalar":
            # Gap-OVL framing: exposed control/sync overlap deficit
            n_scalar_ops = sum(
                1 for op in extract.operations if op.pipe == "PIPE_S"
            )
            n_vec_scalar = sum(
                1 for op in extract.operations
                if op.pipe == "PIPE_S" and op.elements > 1
            )
            model_pct = (ca.model_exposed_control_frac * 100
                         if ca.model_exposed_control_frac is not None else None)
            meas_pct = (ca.measured_scalar_frac * 100
                        if ca.measured_scalar_frac is not None else None)
            # Same-core fraction for the overlap comparison (vs model exposed).
            ovl_meas_pct = (ca.measured_aiv_scalar_frac * 100
                            if ca.measured_aiv_scalar_frac is not None else None)
            ovl_us = ca.gap_ovl_us

            note += (
                f"Scalar is "
                + (f"{meas_pct:.0f}% of measured wall-clock" if meas_pct is not None else "structurally dominant")
                + f" but is {n_vec_scalar}/{n_scalar_ops} vectorizable ops — it is "
                f"exposed control + sync overhead ({ca.n_sync_ops or 0} sync/barrier ops). "
            )
            if model_pct is not None and ovl_meas_pct is not None:
                note += (
                    f"The DES schedule assumes the control path overlaps compute "
                    f"(model exposed {model_pct:.0f}% of the critical path); the "
                    f"hardware serializes it (measured {ovl_meas_pct:.0f}% scalar "
                    f"of AIV core time). "
                )
            if ovl_us is not None:
                note += f"Gap-OVL ≈ {ovl_us:.0f} µs overlap deficit. "
            note += (
                "Action: reduce sync density (multi-buffer), hoist address math "
                "out of the tile loop, fuse tiles to amortize control. "
                "Diagnostic only — bound unchanged."
            )
        ca.note = note
